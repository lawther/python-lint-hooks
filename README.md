...
| [`ML109`](docs/rules/ML109.md) | Cast between two `NewType`s of the same base | Unify the two NewTypes, or name the conversion once: `def f(x: U) -> T: return T(x)` |
...
| [`ML701`](docs/rules/ML701.md) | Timezone silently substituted instead of rejected | Take the zone from the caller that has it, or raise if nobody does — never invent one |
<!-- rules-table-end -->
