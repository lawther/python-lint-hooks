#!/usr/bin/env python3
"""PreToolUse(Bash) guard: reject `--type blocked-by` on `bd dep add` / `bd link`.

`blocked-by` reads like it should be a dependency type -- it is the mirror image of
`blocks`, and `bd dep add` even has a `--blocked-by` *flag* with that exact spelling.
But `--blocked-by` is a flag name (an alias for `--depends-on`, taking an issue ID), not
a member of the `--type` enum. bd's valid dependency types are: blocks, tracks, related,
parent-child, discovered-from, until, caused-by, validates, relates-to, supersedes.
`bd dep add --type blocked-by ...` does not error -- it silently stores an
...
This is not hypothetical: it happened twice in this project's own graph (casey-eoa,
then casey-tusl/casey-5c4/casey-yq7h), both times with the same silent-corruption
signature -- `bd ready` listing beads that should have been blocked. See `bd memories
blocked-by` for the reproduction notes.

Two entry points are checked:

COMMAND LINE. `bd dep add ... --type blocked-by`, `bd dep add ... -t blocked-by`, and
`bd link ... --type blocked-by` / `-t blocked-by`, matched case-insensitively with
underscores normalised to hyphens (`blocked_by` too).
...
scanned for a `"type"` value that normalises to `blocked-by`; `--file -` (stdin) is not
inspectable from a PreToolUse hook and is deliberately let through unchecked.
...
_HOW_TO_FIX = (
    "Use `--type blocks` with the arguments in dependency order (`bd dep add <blocked-id> "
    "<blocker-id>`, or `bd dep <blocker-id> --blocks <blocked-id>`), or drop --type "
    "entirely since `blocks` is the default. If you specifically want issue A to record "
...
def bulk_file_denial(call: list[str]) -> str | None:
    """Denial reason if a `--file` JSONL argument contains a blocked-by edge type."""
    for path_arg in flag_values(call, _FILE_FLAGS):
        ...
        for match in _BULK_TYPE_RE.finditer(text):
            if normalise_type(match.group(1)) == "blocked-by":
                return (
                    f'Blocked: `--file {path_arg}` contains an edge with `"type": "blocked-by"`. {_WHY} {_HOW_TO_FIX}'
                )
    return None
...
def call_denial(call: list[str]) -> str | None:
    ...
    for raw_type in flag_values(call, _TYPE_FLAGS):
        if normalise_type(raw_type) == "blocked-by":
            return f"Blocked: `--type {raw_type}` on `{' '.join(call)}`. {_WHY} {_HOW_TO_FIX}"
