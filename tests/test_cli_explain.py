...
def run_explain(cwd: Path, code: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(  # noqa: S603
        ["uv", "run", "ml-lint", "--explain", code],  # noqa: S607
        capture_output=True,
        text=True,
        cwd=cwd,
        check=False,
...
def test_explain_tip_is_shown(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        ["uv", "run", "ml-lint", "test.py"],  # noqa: S607
        capture_output=True,
        text=True,
        cwd=tmp_path,
        check=False,
    ...
    assert "💡 Tip: For more information and examples, run 'ml-lint --explain ML100'" in result.stdout
